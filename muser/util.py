...
def copy(src, dest):
    try:
        ...
        if e.errno == errno.ENOTDIR:
            ...
            logger.print(f'Directory {src} not copied. Error: %s' % e)
...
def move(src, dest):
    try:
        ...
        # If the error was caused because the source wasn't a directory
        logger.print('Directory not moved. Error: %s' % e)
...
def timeit(within=(-1, -1), without=(-1, -1)):
    ...
    within: only logger.print result within the range of duration
    """
    def actual_decorator(func):
        def wrapper(*args, **kwargs):
            ...
            dur = time.time() - cur
            dur_ms = dur * 1000
            if ((within[0] == -1 or dur_ms >= within[0]) and (within[1] == -1 or dur_ms < within[1]) and (without[0] == -1 or dur_ms < without[0]) and (without[1] == -1 or dur_ms >= without[1])):
                logger.print(f"Function {func.__qualname__} used {dur_ms}ms")
            return res
        return wrapper
